...
OpenShift, is a heavily opinionated enterprise Kubernetes platform. It contains the CNCFR released Kubernetes components unmodified and does respond to kubectl / Kubernetes API calls as any other Kubernetes cluster.  

Notable differences:
  -   authentication - oauth capability on the API server level
  -   OpenShift Web Console (web UI, dashboard)
  ...
  -   use of a locked down RPM / OSTree based system image (using RHEL packages) + Machine Config Operator for host/node OS level changes
  -   the cluster upgrade is specific, automated and includes OS update
  -   not free (60 days unrestricted trial), and you can try OKD, where YMMV (eg Operator Catalog-s)
...
---
## 07 | Kubernetes RBAC and OpenShift Security Context Containers
...
---
## 10 | The Project Template and the Self-Provisioner Role
